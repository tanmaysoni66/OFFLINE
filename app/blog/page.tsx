  ...
  return (
    <div className="min-h-screen pt-24 md:pt-28 pb-12 relative z-20">
      ...
      <section className="section-padding text-center">
        ...
        >
          <h1 className="text-xl md:text-2xl font-bold dark:text-white text-slate-900 mb-4 tracking-tight">
      ...
      <div className="max-w-7xl mx-auto px-4 space-y-10 md:space-y-12">
        {blogPosts.map((column, idx) => (
          <section key={idx}>
            <div className={`inline-flex items-center gap-3 mb-6 px-4 py-1.5 rounded-xl ${column.bg} border dark:border-white/5 border-black/5 ${column.color}`}>
              <column.icon size={18} />
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              {column.posts.map((post, i) => (
                <motion.div 
                   key={i}
                   whileHover={{ y: -5 }}
                   className="glass p-5 rounded-2xl border dark:border-white/5 border-black/5 flex flex-col h-full hover:bg-white/[0.03] transition-all"
                >
                  ...
                  <h3 className="dark:text-white text-slate-900 font-bold text-sm mb-2 leading-snug group-hover:text-primary-start transition-colors">
                    {post.title}
                  </h3>
                  <p className="text-slate-500 text-xs leading-relaxed mb-4 flex-grow">
                    {post.excerpt}
                  </p>
                  <Link href={(post as any).customPath || (post.isExternalPage ? `/articles/${post.id}` : `/blog/${post.id}`)} 
      ...
      <section className="section-padding">
        <div className="max-w-4xl mx-auto glass p-6 md:p-8 rounded-3xl border dark:border-white/10 border-black/10 text-center">
            <h2 className="text-xl font-bold dark:text-white text-slate-900 mb-2">Never Miss an Update</h2>
