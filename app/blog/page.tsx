...
import { BookOpen, TrendingUp, Heart, Mail, Calendar, User, ArrowRight } from "lucide-react";

export const metadata: Metadata = {
  title: "Mushroom Farming Blog | Educational & Health Insights",
  description:
    "Explore practical mushroom farming guides, cultivation techniques, mushroom growing tips, farm setup ideas, harvesting methods, and business insights.",
  alternates: {
...
const blogPosts = [
  ...
  },
  {
    ...
    bg: "bg-green-500/5",
    posts: [
      { id: "mushroom-farming-business-investment-guide", customPath: "/blog/mushroom-farming-business-investment-guide", title: "Mushroom Farming Business: Kam Investment Mein Lakhon Kamane Ka Ek Practical Guide", excerpt: "Agar aap Reddit ya Quora par scroll karte hain, toh aapne ek sawal baar-baar dekha hoga: Kam capital mein sabse accha business kaun sa hai?", date: "August 07, 2026" },
      { id: "mushroom-training-guide-tamil", customPath: "/blog/mushroom-training-guide-tamil", title: "காளான் சாகுபடி பயிற்சி மற்றும் காளான் பண்ணை அமைப்பு: முழுமையான வழிகாட்டி", excerpt: "காளான் சாகுபடி எப்படி தொடங்குவது, என்ன பயிற்சி தேவை, மற்றும் காளான் பண்ணை அமைப்பு (Mushroom Setup) செய்வதற்கான படிப்படியான வழிகாட்டி.", date: "July 20, 2026" },
...
export default function BlogPage() {
  ...
  return (
    <>
      ...
      />
      <main className="pt-24 pb-20 min-h-screen bg-transparent relative overflow-hidden">
        ...
        <article className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <header className="text-center mb-16">
            <h1 className="text-2xl md:text-4xl font-extrabold text-slate-900 dark:text-white mb-4 tracking-tight">
              Mushroom Farming Blog – <span className="text-brand-blue">Guides, Tips & Expert Insights</span>
            </h1>
            <p className="text-[12px] md:text-[14px] text-slate-600 dark:text-slate-400 max-w-3xl mx-auto leading-relaxed">
          ...
          <div className="space-y-16">
            {blogPosts.map((column, idx) => (
              <section key={idx}>
                <div className={`inline-flex items-center gap-3 mb-8 px-6 py-2 rounded-2xl ${column.bg} border dark:border-white/5 border-black/5 ${column.color}`}>
                  <column.icon size={18} />
                  <h2 className="font-black uppercase tracking-widest text-[11px] md:text-[12px]">{column.category}</h2>
                </div>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {column.posts.map((post: any, i) => {
                    const postLink = post.customPath || (post.isExternalPage ? `/articles/${post.id}` : `/blog/${post.id}`);
                    return (
                      <div
                        key={i}
                        className="glass p-6 md:p-8 rounded-[2rem] border dark:border-white/5 border-black/5 flex flex-col h-full hover:bg-white/[0.03] hover:border-brand-blue/30 transition-all group"
                      >
                        <div className="flex items-center gap-4 text-[9px] text-slate-500 font-bold uppercase tracking-widest mb-4">
                          <span className="flex items-center gap-1"><Calendar size={10} /> {post.date}</span>
                          <span className="flex items-center gap-1"><User size={10} /> Expert Team</span>
                        </div>
                        <h3 className="dark:text-white text-slate-900 font-bold text-[14px] md:text-[15px] mb-3 leading-snug group-hover:text-brand-blue transition-colors">
                          <Link href={postLink}>
                            {post.title}
                          </Link>
                        </h3>
                        <p className="text-slate-500 text-[11px] leading-relaxed mb-6 flex-grow">
                          {post.excerpt}
                        </p>
                        <Link
                          href={postLink}
                          className="text-brand-blue font-bold text-[10px] uppercase tracking-widest flex items-center gap-2 mt-auto w-fit border-b border-brand-blue/20 pb-1 group-hover:gap-3 transition-all"
                        >
          ...
          <section className="mt-20">
            <div className="max-w-4xl mx-auto glass p-8 md:p-12 rounded-[3rem] border dark:border-white/10 border-black/10 text-center">
              <div className="flex justify-center mb-4">
                <Mail size={32} className="text-brand-blue" />
              </div>
              <h2 className="text-2xl font-bold dark:text-white text-slate-900 mb-3">Never Miss an Update</h2>
              <p className="text-[12px] dark:text-slate-400 text-slate-600 mb-8 max-w-lg mx-auto">
              ...
              >
                <input 
                  ...
                  className="w-full bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 text-[12px] rounded-xl px-5 py-3 text-slate-900 dark:text-white outline-none focus:border-brand-blue transition-colors"
                />
                ...
                <button 
                  type="submit" 
                  name="subscribe" 
                  className="w-full sm:w-auto bg-brand-blue hover:bg-brand-blue/90 text-white font-semibold text-[12px] px-8 py-3 rounded-xl transition-colors whitespace-nowrap"
                >
                  Subscribe
                </button>
              </form>
